            ...
            <div class="border border-[#c9a84c]/20 p-6 reveal" style="transition-delay:{{ $loop->index*100 }}ms">
                <div class="flex items-start justify-between gap-4">
                    <div class="flex-1">
                        ...
                        <p class="text-[#fdf5e4]/40 text-sm">{{ $event->starts_at->format('H:i') }}@if($event->ends_at) — {{ $event->ends_at->format('H:i') }} WIB@endif</p>
                        @endif
                        @if($event->venue)<p class="text-[#fdf5e4]/60 text-sm mt-3">{{ $event->venue }}</p>@endif
                        @if($event->address)<p class="text-[#fdf5e4]/30 text-xs mt-1">{{ $event->address }}</p>@endif
