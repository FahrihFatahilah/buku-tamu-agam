...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=EB+Garamond:ital,wght@0,400;0,500;1,400&family=DM+Sans:wght@300;400&display=swap" rel="stylesheet">
    @vite(['resources/css/app.css', 'resources/js/app.js'])
    <style>
        body { font-family: 'DM Sans', sans-serif; background: #ffffff; color: #111; }
...
<div class="fixed bottom-4 right-4 z-50" x-data="{ playing: false, audio: null }" x-init="audio = $refs.audio">
    <button @click="playing ? (audio.pause(), playing=false) : audio.play().then(()=>playing=true).catch(()=>{})"
        class="w-9 h-9 border border-stone-300 text-stone-500 flex items-center justify-center hover:border-stone-500 transition-colors text-xs">
...
<div class="fixed inset-0 z-40 bg-white flex flex-col items-center justify-center text-center px-8"
    ...
    <p class="text-stone-400 text-xs tracking-[0.3em] uppercase mb-8">Undangan Pernikahan</p>
    <h1 class="font-display text-4xl text-stone-900 mb-1">{{ $wedding->groom_nickname ?: $wedding->groom_name }}</h1>
    <p class="text-stone-300 text-xl mb-1">&</p>
    <h1 class="font-display text-4xl text-stone-900 mb-8">{{ $wedding->bride_nickname ?: $wedding->bride_name }}</h1>
    @if($guest)<p class="text-stone-400 text-sm mb-6">{{ $guest->name }}</p>@endif
    <button @click="opened=true"
        class="px-6 py-2.5 border border-stone-300 text-stone-600 text-xs tracking-widest uppercase hover:border-stone-500 transition-colors">
        Buka
    </button>
</div>
@endif
