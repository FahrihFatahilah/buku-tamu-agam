<section id="countdown" class="py-20 px-6 bg-white" x-data="countdown('{{ $wedding->date->format('Y-m-d') }}')">
    <div class="max-w-lg mx-auto text-center">
        <p class="text-stone-400 text-xs tracking-[0.3em] uppercase mb-10 reveal">Menuju Hari Bahagia</p>
        <div class="grid grid-cols-4 gap-6">
            <template x-if="!passed"><template x-for="unit in units" :key="unit.label"><div class="reveal"><div class="text-4xl font-display text-stone-900" x-text="unit.value"></div><div class="text-xs text-stone-300 tracking-widest mt-1" x-text="unit.label"></div></div></template></template>
            <template x-if="passed"><div class="col-span-4"><p class="font-display text-2xl text-stone-900">Hari yang dinantikan telah tiba</p></div></template>
        </div>
        <p class="mt-8 text-stone-300 text-sm tracking-wider reveal">{{ $wedding->date->translatedFormat('l, d F Y') }}</p>
    </div>
</section>
<script>
