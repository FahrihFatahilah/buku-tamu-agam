    ...
    <div class="max-w-2xl mx-auto">
        ...
        <div class="space-y-10">
            ...
            <div class="reveal" style="transition-delay:{{ $loop->index*100 }}ms">
                <div class="border-l-2 border-stone-200 pl-5 py-2">
                    ...
                    @if($event->venue)
                    <p class="text-stone-500 text-sm mt-2">{{ $event->venue }}</p>
                    @endif

                    @if($event->address)
                    <p class="text-stone-300 text-xs mt-1">{{ $event->address }}</p>
                    @endif

                    @if($event->maps_url && !$event->maps_embed)
                    <a href="{{ $event->maps_url }}" target="_blank" rel="noopener"
                        class="inline-block mt-2 text-xs text-stone-400 hover:text-stone-600 underline underline-offset-2">
                        Lihat Peta →
                    </a>
                    @endif
                </div>
                ...
                @if($event->maps_embed)
                <div class="w-full border border-stone-200 mt-3 overflow-hidden" style="height:260px;">
                ...
                <a href="{{ $event->maps_url }}" target="_blank" rel="noopener"
                    class="inline-block mt-1 text-xs text-stone-400 hover:text-stone-600 underline underline-offset-2">
                    Buka di Google Maps →
                </a>
                @endif
            </div>
