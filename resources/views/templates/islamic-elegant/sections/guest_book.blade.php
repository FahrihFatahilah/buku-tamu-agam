...
<section id="guest_book" class="py-20 px-6 bg-[#1a4a2e]" x-data="guestBook('{{ $gbUrl }}','{{ csrf_token() }}')">
    <div class="max-w-xl mx-auto">
        <p class="text-[#c9a84c] text-xs tracking-[0.4em] uppercase mb-3 text-center reveal">Ucapan & Doa</p>
        <h2 class="font-display text-2xl text-white text-center mb-10 reveal">Buku Tamu</h2>
        ...
        <form @submit.prevent="submit" class="space-y-4 mb-12 reveal" x-show="!sent">
            <input type="text" x-model="form.name" placeholder="Nama Anda" value="{{ $guest?->name }}" {{ $guest?'readonly':'' }} class="w-full px-4 py-3 bg-white/5 border border-white/10 text-white text-sm placeholder-white/30 focus:outline-none focus:border-[#c9a84c]/50 transition-colors">
            <textarea x-model="form.message" rows="3" placeholder="Tulis ucapan dan doa..." class="w-full px-4 py-3 bg-white/5 border border-white/10 text-white text-sm placeholder-white/30 focus:outline-none focus:border-[#c9a84c]/50 transition-colors resize-none"></textarea>
        ...
        <div class="space-y-4">
            ...
            <div class="border-b border-white/10 pb-4 last:border-0">
                <div class="flex items-baseline justify-between gap-2 mb-1">
                    <p class="font-display text-sm text-white">{{ $entry->name }}</p>
                    <time class="text-white/30 text-xs shrink-0">{{ $entry->created_at->diffForHumans() }}</time>
                </div>
                <p class="text-white/50 text-sm leading-relaxed">{{ $entry->message }}</p>
            </div>
            @endforeach
        </div>
    </div>
