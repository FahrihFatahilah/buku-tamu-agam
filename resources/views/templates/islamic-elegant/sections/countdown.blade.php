<section id="countdown" class="py-20 px-6 bg-[#1a4a2e]" x-data="countdown('{{ $wedding->date->format('Y-m-d') }}')">
    <div class="max-w-lg mx-auto text-center">
        <p class="text-[#c9a84c] text-xs tracking-[0.4em] uppercase mb-10 reveal">Menuju Hari Bahagia</p>
        <div class="grid grid-cols-4 gap-4">
            <template x-if="!passed"><template x-for="unit in units" :key="unit.label"><div class="reveal"><div class="text-4xl font-display text-white" x-text="unit.value"></div><div class="text-xs text-white/40 tracking-widest mt-1" x-text="unit.label"></div></div></template></template>
            <template x-if="passed"><div class="col-span-4"><p class="font-display text-2xl text-white">Hari yang dinantikan telah tiba</p></div></template>
        </div>
        <p class="mt-8 text-white/40 text-sm reveal">{{ $wedding->date->translatedFormat('l, d F Y') }}</p>
    </div>
</section>
<script>
