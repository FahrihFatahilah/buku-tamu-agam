...
<head>
    ...
    <style>
        :root { --islamic-green: #1a4a2e; --islamic-gold: #c9a84c; --islamic-cream: #fdf8f0; }
        body { font-family: 'Nunito', sans-serif; background: var(--islamic-cream); color: #1a1a1a; }
...
<div class="fixed bottom-5 right-5 z-50" x-data="{ playing: false, audio: null }" x-init="audio = $refs.audio">
    <button @click="playing ? (audio.pause(), playing=false) : audio.play().then(()=>playing=true).catch(()=>{})"
        class="w-10 h-10 rounded-full bg-[#1a4a2e] text-white flex items-center justify-center hover:bg-[#0f3020] transition-colors text-xs">
...
<div class="fixed inset-0 z-40 bg-[#1a4a2e] flex flex-col items-center justify-center text-center px-8"
    ...
    <p class="text-[#c9a84c] text-sm mb-4 font-display italic">بِسْمِ اللَّهِ الرَّحْمَنِ الرَّحِيم</p>
    <p class="text-white/50 text-xs tracking-[0.3em] uppercase mb-6">Undangan Pernikahan</p>
    <h1 class="font-display text-4xl text-white font-light mb-1">{{ $wedding->groom_nickname ?: $wedding->groom_name }}</h1>
    <p class="text-[#c9a84c] text-xl mb-1">&</p>
    <h1 class="font-display text-4xl text-white font-light mb-8">{{ $wedding->bride_nickname ?: $wedding->bride_name }}</h1>
    @if($guest)<p class="text-white/50 text-sm mb-6">Kepada: {{ $guest->name }}</p>@endif
    <button @click="opened=true"
        class="px-8 py-3 border border-[#c9a84c]/50 text-[#c9a84c] text-xs tracking-widest uppercase hover:border-[#c9a84c] transition-colors">
        Buka Undangan
    </button>
</div>
@endif
