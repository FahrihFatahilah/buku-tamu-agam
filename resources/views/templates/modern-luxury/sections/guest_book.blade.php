...
<section id="guest_book" class="py-20 px-6 bg-white" x-data="guestBook('{{ $gbUrl }}','{{ csrf_token() }}')">
    <div class="max-w-xl mx-auto">
        <p class="text-[#c9a96e] text-xs tracking-[0.4em] uppercase mb-3 text-center reveal">Ucapan & Doa</p>
        <h2 class="font-display text-2xl text-[#1a1a1a] text-center mb-10 reveal">Buku Tamu</h2>
        ...
        <form @submit.prevent="submit" class="space-y-4 mb-12 reveal" x-show="!sent">
            <input type="text" x-model="form.name" placeholder="Nama Anda" value="{{ $guest?->name }}" {{ $guest?'readonly':'' }} class="w-full px-4 py-3 border border-stone-200 text-sm focus:outline-none focus:border-[#c9a96e]/50 transition-colors">
            <textarea x-model="form.message" rows="3" placeholder="Tulis ucapan dan doa..." class="w-full px-4 py-3 border border-stone-200 text-sm focus:outline-none focus:border-[#c9a96e]/50 transition-colors resize-none"></textarea>
            <button type="submit" :disabled="loading" class="w-full py-3 bg-[#1a1a1a] text-white text-xs tracking-[0.2em] uppercase hover:bg-stone-800 transition-colors disabled:opacity-50">
                <span x-show="!loading">Kirim Ucapan</span><span x-show="loading">Mengirim...</span>
            </button>
        </form>
        <div class="space-y-4">
            ...
            <div class="border-b border-stone-100 pb-4 last:border-0">
                <div class="flex items-baseline justify-between gap-2 mb-1">
                    <p class="font-display text-sm text-[#1a1a1a]">{{ $entry->name }}</p>
