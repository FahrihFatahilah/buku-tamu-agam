<label class="block">
    ...
    <select
        ...
        @change="$store.builder.fieldWrite('{{ $field['name'] }}', $event.target.value, '{{ $target }}')"
        ...
        <option value="" class="bg-stone-800">—</option>
        @foreach($field['options'] ?? [] as $option)
        <option value="{{ $option['value'] }}" class="bg-stone-800">{{ $option['label'] ?? $option['value'] }}</option>
        @endforeach
    </select>
</label>
